...
#       )
#     }) %>% 
#       dplyr::filter(final_compartment %in% c("exposed_diagnosed", "infected_asymptomatic_diagnosed",
#                              "infected_presymptomatic_diagnosed", "infected_symptomatic_diagnosed",
#                              "recovered_diagnosed"))
#   })
#   
#   out_probabilities <- out_df %>% 
#     dplyr::filter(time == isolation_period) %>% 
#     dplyr::group_by(final_compartment) %>% 
#     dplyr::summarise(total = sum(value)) %>% 
#     dplyr::mutate(probability = total/sample_size,
#            starting_compartment = starting_compartment) %>% 
#     dplyr::select(starting_compartment, final_compartment, total, probability)
#   
#   probabilities_as_vector <- out_probabilities %>%
#     dplyr::select(final_compartment, probability) %>% tibble::deframe()
#   
